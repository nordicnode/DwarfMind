...
|---|---|---|
| `reflex_defense` | Tactical defense | Auto-pulls registered defense levers when hostile units are detected. Lever names are matched using word-boundary rules (e.g. `main_gate` matches, `floodgate` does not) to prevent false positives on non-defense levers. Pulls are state-aware: an already-sealed gate is never re-pulled, preventing the lever from being toggled back open mid-siege. |
...
| `reflex_tantrum_watch` | Mental health | Early-warning tantrum detection independent of raw stress score. Monitors citizens at a lower stress floor (10 000 — top of DF's neutral band) and inspects bad-thought lists for high-weight persistent negatives (witnessed death, lost loved one, tribute demand). Queues a fine-meal consolation thought and logs a targeted warning per at-risk citizen. Catches dwarves actively tantrumming or going berserk for immediate escalation. |
...
| `reflex_potash_chain` | Agricultural yield | Maintains a potash buffer for crop fertilization. Crawls the dependency chain: if potash is low it queues `MakePotashFromAsh` at the Ashery; if ash is low it burns wood logs while respecting an ash floor reserve for `reflex_soap_chain`. |
...
| `reflex_trap_logistics` | Engineering buffer | Maintains a minimum reserve of 5 free mechanisms (`TRAPPARTS`) via `ConstructMechanisms` orders using stone. Prevents `reflex_defense` and `reflex_hydrology` from stalling due to mechanism shortages. |
...
### 🧠 Brain & Spatial Planning
| Script | Category | Role & Behavior |
|---|---|---|
| `reflex_orchestrator` | Macro-goal FSM | Arbitrates fort-wide `PEACE` / `SIEGE` / `DISTRESS_FAMINE` / `QUARANTINE` states from sensor snapshots and throttles reflex cadences per category (suspends luxury during crises, halts construction during sieges). FSM state persists across save/load. |
| `build_layer` | Spatial planning | Scans terrain in round-robin windows, generates dig blueprints (workshop block, residential nodes, stockpile, admin hall) anchored on the embark wagon, issues dig designations, and queues furniture construction — all gated by `dry_run`. Furniture placement fails soft (self-disables with a warning) when DFHack's `build` tool is unavailable. |
